...
class Board:
    ...
    @classmethod
    def fromFilename(cls, file : str, level : int, goal : list):
        """
        This classmethod is used to relicate overloading the init function
        by allowing devleopers to create a board from a file which contains
        the data as well as just by using a list with the data in.

        :param file: The path to the file which contains the board data.
        :type file: str
        :param level: The level (depth) of the board.
        :type level: int
        :param goal: The goal board / state.
        ...
        :return : The object that has been initialised.
        :rtype: Board
        """
        board = []
        with open(file, "r") as file:
            for line in file:
                ...
                if len(row) > 3:
                    raise Exception("ERROR - Starting board in given file has too many nums in row.")
        
        if len(board) > 3:
            raise Exception("ERROR - Starting board in given file has too many rows.")

        return cls(board, level, goal)
